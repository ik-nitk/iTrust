...
def create_new_beneficiary(repo,fname,mname,lname,phone,email, created_by):
    try:
        id = repo.create_beneficiary(fname,mname,lname,phone,email, created_by)
...
def update_beneficiary(repo, beneficiary_id,fname,mname,lname,phone,email, updated_by):
    try:
        beneficiary = repo.update_beneficiary(beneficiary_id,fname,mname,lname,phone,email, updated_by)
        return ResponseSuccess(beneficiary)
    except Exception as exc:
        return ResponseFailure(ResponseTypes.SYSTEM_ERROR, exc)
